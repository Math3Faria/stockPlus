...
export class ProdutoController {
    ...
    criar = async (req: Request, res: Response) => {
        try {
            const { nomeProduto, valor, idCategoria, idFornecedor, quantidade, qtdMax, qtdMin } = req.body;
            if (!nomeProduto || !valor || !idCategoria || !idFornecedor || quantidade === undefined || qtdMax === undefined || qtdMin === undefined) {
            ...
            const novo = await this._service.criar(nomeProduto, Number(valor), Number(idCategoria), Number(idFornecedor), imagemProduto, Number(quantidade), Number(qtdMax), Number(qtdMin));
    ...
    editar = async (req: Request, res: Response) => {
        try {
            ...
            const imagemProduto = req.file?.filename;
            if (!nomeProduto || !valor || !idCategoria || !idFornecedor || !imagemProduto || quantidade === undefined || qtdMax === undefined || qtdMin === undefined) {
                return res.status(400).json({ message: "Todos os campos são obrigatórios" });
            }
            const alterado = await this._service.editar(idProduto, nomeProduto, Number(valor), Number(idCategoria), Number(idFornecedor), imagemProduto, Number(quantidade), Number(qtdMax), Number(qtdMin));
